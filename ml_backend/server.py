...
def get_args():
    ...
    root_parser.add_argument("--root-dir", dest="root_dir", default=".", help="Projects root directory")
    ...
    parser_init = subparsers.add_parser(
        "init", aliases=["create"], help="Initialize Label Studio", parents=[root_parser]
    )
    parser_init.add_argument("project_name", help="Path to directory where project state will be initialized")
    ...
    parser_init.add_argument(
        "--force", dest="force", action="store_true", help="Force recreating the project if exists"
    )

    # start sub-command parser
    parser_start = subparsers.add_parser("start", help="Initialize Label Studio", parents=[root_parser])
    parser_start.add_argument("project_name", help="Path to directory where project state will be initialized")
    ...
    parser_deploy.add_argument("provider", help="Provider where to deploy")
    parser_deploy.add_argument("project_name", help="Path to directory where project state will be initialized")
    ...
    parser_deploy.add_argument("--gcp-project-id", dest="gcp_project", help="GCP project ID")
    parser_deploy.add_argument("--gcp-region", dest="gcp_region", help="GCP region")
    parser_deploy.add_argument(
        "--label-studio-host",
        dest="label_studio_host",
        default="https://app.heartex.com",
        help="Label Studio hostname",
    )
    parser_deploy.add_argument(
        "--label-studio-api-key", dest="label_studio_api_key", required=True, help="Label Studio API key"
    )
...
def create_dir(args):
    ...
    if os.path.exists(output_dir) and args.force:
        ...
        print(
            Fore.RED + f'Model directory "{output_dir}" already exists. Please remove it or run:\n'
    ...
    if model_def_in_path(script_path):
        ...
        if len(model_classes) > 1:
            raise ValueError(
                ...
                "{script}:{model_class}".format(
                    num=len(model_classes), script=script_path, model_class=model_classes[0]
                )
            )
    ...
    with open(wsgi_script_file) as f:
        wsgi_script = f.read()
    wsgi_script = wsgi_script.format(script=os.path.splitext(script_base_name)[0], model_class=model_class)
...
def start_server(args, subprocess_params):
    ...
    if args.basic_auth_user and args.basic_auth_pass:
        cmd_args = ["--basic-auth-user", args.basic_auth_user, "--basic-auth-pass", args.basic_auth_pass]
    ...
    subprocess.check_output(
        " ".join(
            [
                ...
                region,
                "--update-env-vars",
                f"LABEL_STUDIO_ML_BACKEND_V2=1,LABEL_STUDIO_HOSTNAME={args.label_studio_host},LABEL_STUDIO_API_KEY={args.label_studio_api_key}",
            ]
        ),
        input=b"y",
        shell=True,
