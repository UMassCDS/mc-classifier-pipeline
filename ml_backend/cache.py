...
class SqliteCache(BaseCache):
    ...
    def __getitem__(self, project_id_key):
        ...
        with self.lock, sqlite3.connect(self.db_name) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT value FROM cache WHERE project_id = ? AND key = ?;", (project_id, key))
    ...
    def __setitem__(self, project_id_key, value):
        ...
        with self.lock, sqlite3.connect(self.db_name) as conn:
            cursor = conn.cursor()
            cursor.execute("REPLACE INTO cache (project_id, key, value) VALUES (?, ?, ?);", (project_id, key, value))
    ...
    def __delitem__(self, project_id_key):
        ...
        with self.lock, sqlite3.connect(self.db_name) as conn:
            cursor = conn.cursor()
            cursor.execute("DELETE FROM cache WHERE project_id = ? AND key = ?;", (project_id, key))
        self.__getitem__.cache_clear()
    ...
    def __contains__(self, project_id_key):
        ...
        with self.lock, sqlite3.connect(self.db_name) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT 1 FROM cache WHERE project_id = ? AND key = ?;", (project_id, key))
