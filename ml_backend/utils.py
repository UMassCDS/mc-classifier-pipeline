...
def get_single_tag_keys(parsed_label_config, control_type, object_type):
    ...
    assert len(parsed_label_config) == 1, (
        "Please check your Labeling Interface configuration: At least one control tag should be in the labeling config."
    )  # noqa
...
def match_labels(input: str, labels: List[str]) -> List[str]:
    ...
    for pred in predicted_classes:
        scores = list(map(lambda label: difflib.SequenceMatcher(None, pred, label).ratio(), labels))
...
def is_preload_needed(url):
    ...
    return is_uploaded_file or is_local_storage_file or is_cloud_storage_file or is_valid_url(url) or path_exists
